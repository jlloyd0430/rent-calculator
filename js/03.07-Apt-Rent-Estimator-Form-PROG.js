...
let doorman = document.getElementById('doorman');
...
//check if flat fee checkboxes are checked

if(parking.checked){
  rent += Number(parking.value); // +350
}

if(gym.checked){
  rent += Number(gym.value);// 100
}

if(doorman.checked){
  rent += rent * Number(doorman.value); // + 10%
}

if(view.checked){
  rent += Number(view.value); // +25%
}

if(laundry.checked){
  rent += Number(laundry.value);// +50
}
